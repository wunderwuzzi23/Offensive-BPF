...
int handle_exit_getdents64(struct trace_event_raw_sys_exit *ctx)
{
    ...
    while (i < 256)   // limitation for now, only examine the first 256 entries
    {
        ...
        //bpf_printk("target pattern: %s", PATTERN);

        bool match = true;
        // char* target = "wunderwuzzi\0";    
        // for (int j=0; j < PATTNER_LEN; j++)
        // {
        //     if (target[j] != d_name[j])
        //     {
        //         match = false;
        //         break;
        //     }
